...
<nav>
  ...
  <a href="{$page.data.locale === 'fr' ? '' : `/${$page.data.locale}`}/projets" class:current={true} rel=prefetch>
    <h4>{director.fields.name}</h4>
  </a>
  {:else}
  <a href="{$page.data.locale === 'fr' ? '' : `/${$page.data.locale}`}/projets?director={director.fields.tagIdentifier}" rel=prefetch>
    <h4>{director.fields.name}</h4>
  </a>
  {/if}
  {/each}
